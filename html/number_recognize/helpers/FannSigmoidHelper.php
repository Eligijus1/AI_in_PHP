...
class FannSigmoidHelper
{
    public function trainAndTest(int $maxEpochs)
    {
        ...
        if (!is_file(FannHelper::TRAINING_LOG_FILE)) {
            file_put_contents(FannHelper::TRAINING_LOG_FILE, "DateTime,Epoch,MSE");
        } else {
        ...
        if ($fann) {
            ...
            while (($psudoMseResult > $desiredError) && ($currentEpoch <= $maxEpochs)) {
                ...
                HelperFunctions::printInfo(sprintf("Epoch: %' 6s; MSE: %' 20s;", $currentEpoch, $psudoMseResult));
                fwrite($logFile,
                    PHP_EOL . date_format(new DateTime(), 'Y-m-d H:i:s') . ",$currentEpoch,$psudoMseResult");
                ...
                if (($epochsSinceLastSave >= $epochsBetweenSaves) && ($psudoMseResult < $bestMse)) {
                    ...
                    $temSaveFile = FannHelper::NETWORK_CONFIGURATION_FILE . date_format(new DateTime(), '_Y.m.d_H-i-s');
